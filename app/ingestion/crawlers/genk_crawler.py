...
class GenkCrawler(BaseCrawler):
    ...
    article_url_pattern = re.compile(r"^https://genk\.vn/.+?-\d+\.chn$")
    ...
    def parse_article(self, html: str, url: str) -> ParsedArticle:
        ...
        content_nodes = soup.select(".knc-content p, .newsContent p, .detail-content p")
        category_nodes = soup.select(".kbws-breadcrumb a, .breadcrumb li a")
        author_nodes = soup.select(".author, .detail-author")
        tag_nodes = soup.select(".tags a, .detail-tags a")
        title_node = soup.select_one("h1.kbwc-title") or soup.select_one("h1")
        summary_node = soup.select_one(".knc-sapo") or soup.select_one(".sapo")
        ...
        return ParsedArticle(
            ...
            publish_time=parse_datetime(
                ...
                or (soup.select_one(".kbwcm-time") and soup.select_one(".kbwcm-time").get_text(" ", strip=True))
            ),
